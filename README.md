...
`pnpm test` executes only `.ts`/`.tsx` files (`vitest.config.ts`'s include list).
That is Q-08 read the other way round: `vextrus/no-forbidden-escapes` lints the ts and
tsx globs, so a test file in any other extension would be one the runner executes and
