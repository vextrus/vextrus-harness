...
/**
 * Reachability over a unix domain socket. A local Postgres cluster may listen
 * only on `/var/run/postgresql/.s.PGSQL.<port>`; a TCP-only probe would then
 * call a perfectly healthy machine unreachable.
 */
export function socketReachable(path, timeoutMs = 2000) {
  return new Promise((resolve) => {
    ...
    socket.setTimeout(timeoutMs);
    socket.once('connect', () => finish({ ok: true, detail: `accepting connections on ${path}` }));
    socket.once('timeout', () => finish({ ok: false, detail: `no answer from ${path} within ${timeoutMs}ms` }));
    socket.once('error', (error) =>
      finish({ ok: false, detail: `unreachable on ${path} (${error.code ?? error.message})` }),
    );
  });
}
