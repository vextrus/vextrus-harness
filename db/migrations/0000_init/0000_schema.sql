...
ALTER TABLE "seam_probe_rows" ADD CONSTRAINT "seam_probe_rows_tenant_fk" FOREIGN KEY ("tenant_id") REFERENCES "public"."tenants"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
CREATE POLICY "seam_probe_ledger_tenant_isolation" ON "seam_probe_ledger" AS PERMISSIVE FOR ALL TO public USING ((
...
CREATE POLICY "seam_probe_rows_tenant_isolation" ON "seam_probe_rows" AS PERMISSIVE FOR ALL TO public USING ((
  "tenant_id"::text = current_setting('app.tenant_id', true)
  or current_setting('app.system', true) = 'on'
)) WITH CHECK ((
  "tenant_id"::text = current_setting('app.tenant_id', true)
...
CREATE POLICY "tenants_tenant_isolation" ON "tenants" AS PERMISSIVE FOR ALL TO public USING ((
